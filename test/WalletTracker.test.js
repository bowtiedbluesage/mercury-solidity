...
contract('WalletTracker', function(accounts) {
    ...
    it('should let the tracked wallet send a transaction', async function() {
        await instance.recordTransaction({ from: trackedWallet, value: web3.utils.toWei("1", "ether")});
    ...
    it('should reject a transaction from non-tracked wallet', async function() {
        try {
            await instance.recordTransaction({ from: nonTrackedWallet, value: web3.utils.toWei("1", "ether")});
    ...
    await instance.recordTransaction({ from: trackedWallet, value: valueToSend });
    ...
    assert.equal(transaction.value.toString(), valueToSend, "transaction value should match sent value");
});
